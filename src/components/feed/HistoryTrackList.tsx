      ...
      {items.map((item, index) => (
        <div key={item.id} className="flex items-center gap-4 p-4 transition-colors hover:bg-panel">
          <button
            ...
            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-ink text-canvas transition hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink focus-visible:ring-offset-2"
            aria-label={`Play ${item.track.title}`}
          >
            <Play className="h-4 w-4 fill-current" />
          </button>
          <div className="min-w-0 flex-1">
            <Link href={`/track/${item.track.id}`} className="truncate rounded-sm font-medium text-ink hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink">
              {item.track.title}
            </Link>
            <p className="truncate text-sm text-muted">{item.track.artist}</p>
          </div>
          <div className="hidden text-right text-xs text-muted sm:block">
            <p>{item.source || "Infini"}</p>
