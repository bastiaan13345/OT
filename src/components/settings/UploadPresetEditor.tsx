...
export function UploadPresetEditor({
  ...
  const textControl = (field: (typeof stringFields)[number]) => {
    ...
    if (field.kind === "textarea") {
      return <div className="space-y-2"><div className="flex items-center justify-between gap-2"><label htmlFor={`preset-${field.key}`} className="text-sm font-medium text-zinc-200">{field.label}</label>{removeButton(field.key, field.label)}</div><textarea id={`preset-${field.key}`} value={value} onChange={(event) => setValue(field.key, event.target.value)} disabled={pending} rows={3} maxLength={5000} className="upload-control-focus w-full resize-y rounded-lg border border-white/15 bg-surface-900 px-4 py-2.5 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50" /></div>;
    }
    if (field.kind === "number") {
      return <div className="space-y-2"><div className="flex items-center justify-between gap-2"><label htmlFor={`preset-${field.key}`} className="text-sm font-medium text-zinc-200">{field.label}</label>{removeButton(field.key, field.label)}</div><input id={`preset-${field.key}`} type="number" min="0" step="0.01" value={value} onChange={(event) => setValue(field.key, event.target.value)} disabled={pending} className="upload-control-focus w-full rounded-lg border border-white/15 bg-surface-900 px-4 py-2.5 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50" /></div>;
    }

    const suggested = field.key in suggestions;
    return <div className="flex items-end gap-2"><div className="min-w-0 flex-1">{suggested ? <SuggestionField label={field.label} value={value} suggestions={suggestions[field.key as keyof typeof suggestions]} onChange={(next) => setValue(field.key, next)} disabled={pending} /> : <><label htmlFor={`preset-${field.key}`} className="mb-2 block text-sm font-medium text-zinc-200">{field.label}</label><input id={`preset-${field.key}`} value={value} onChange={(event) => setValue(field.key, event.target.value)} disabled={pending} maxLength={200} className="upload-control-focus w-full rounded-lg border border-white/15 bg-surface-900 px-4 py-2.5 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50" /></>}</div>{removeButton(field.key, field.label)}</div>;
  };

  return (
    <form onSubmit={(event) => { event.preventDefault(); onSave(draft, enabled); }} className="space-y-5 rounded-2xl border border-rose-400/30 bg-surface-950 p-5 shadow-2xl shadow-black/25" aria-label={draft.id ? "Edit upload preset" : "New upload preset"}>
      <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold text-white">{draft.id ? "Edit preset" : "New preset"}</h2><p className="mt-1 text-sm text-zinc-400">Choose exactly which values this preset should apply.</p></div><button type="button" onClick={onCancel} disabled={pending} className="upload-control-focus rounded-lg p-2 text-zinc-400 transition hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-50" aria-label="Cancel editor"><X aria-hidden="true" className="h-5 w-5" /></button></div>
      <div><label htmlFor="preset-name" className="mb-2 block text-sm font-medium text-zinc-200">Preset name</label><input id="preset-name" value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} disabled={pending} required maxLength={80} className="upload-control-focus w-full rounded-lg border border-white/15 bg-surface-900 px-4 py-2.5 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50" /></div>
      <div className="grid gap-4 md:grid-cols-2">{stringFields.map((field) => <div key={field.key}>{textControl(field)}</div>)}</div>
      <div className="grid gap-3 border-t border-white/10 pt-4 md:grid-cols-2">{booleanFields.map((field) => enabled.has(field.key) ? <div key={field.key} className="flex items-center gap-2 rounded-lg border border-white/10 px-3 py-2"><div className="min-w-0 flex-1"><SwitchField name={field.key} label={field.label} checked={Boolean(draft.values[field.key])} onChange={(next) => setValue(field.key, next)} disabled={pending} /></div>{removeButton(field.key, field.label)}</div> : <button key={field.key} type="button" onClick={() => include(field.key)} disabled={pending} className="upload-control-focus rounded-lg border border-white/15 px-3 py-2 text-left text-sm text-zinc-200 transition hover:border-rose-400/60 hover:text-white disabled:cursor-not-allowed disabled:opacity-50" aria-label={`Include ${field.label.toLowerCase()}`}>Include {field.label}</button>)}</div>
      <div className="flex justify-end gap-3"><button type="button" onClick={onCancel} disabled={pending} className="upload-control-focus rounded-lg border border-white/15 px-4 py-2 text-sm font-medium text-zinc-200 transition hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-50">Cancel</button><button type="submit" disabled={pending} className="upload-control-focus inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-rose-500 disabled:cursor-not-allowed disabled:opacity-50"><Save aria-hidden="true" className="h-4 w-4" />{pending ? "Saving…" : draft.id ? "Save preset" : "Create preset"}</button></div>
    </form>
  );
}
